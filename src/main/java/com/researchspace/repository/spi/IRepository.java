...
/**
 * Top level interface for data repository integration. This is loosely an an
 * abstraction over our repository implementations - Dataverse, Figshare and
 * DSpace.
 * 
 * @author radams
 *
...
public interface IRepository {
	/**
	 * Null implementation that does nothing, but avoids the need for null-checks
	 * when using in a test environment.
	 */
	 ...
	 * 
	 * @param depositor
	 *            the User performing the deposit
	 * @param toDeposit
	 *            the file to deposit to the archive
	 * @param metadata
	 *            a {@link SubmissionMetadata}
	 * @throws RepositoryDepositException
	 *             - wraps any low level implementation exception.
	 */
	RepositoryOperationResult submitDeposit(IDepositor depositor, File toDeposit, SubmissionMetadata metadata,
			RepositoryConfig repoCfg);

	/**
	 * Should be a read-only, quick operation that verifies connection to the
	 * repository
	 * 
	 * @return A success message if connection is OK
	 * @throws a
	 *             runtime {@link IllegalStateException} if connection not OK
	 */
